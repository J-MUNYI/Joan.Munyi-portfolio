const contacts = [
  { label: "Email", value: "joan.munyi.dev@gmail.com", href: "mailto:joan.munyi3@gmail.com" },
...
export default function Contact() {
  return (
    <section id="contact" className="fade-up" style={{ background: "var(--bg2)", padding: "6rem 2.5rem" }}>
      <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
        ...
        <h2 style={{ fontFamily: "'DM Serif Display',serif", fontSize: "clamp(2rem,4vw,3rem)", color: "var(--text)", marginBottom: "1rem" }}>Let's Build Something</h2>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: "1.5rem" }}>
          <div style={{ gridColumn: "1 / -1", background: "var(--surface)", border: "1px solid var(--border2)", borderRadius: "16px", padding: "2.5rem", textAlign: "center" }}>
            <h3 style={{ fontFamily: "'DM Serif Display',serif", fontSize: "1.8rem", color: "var(--text)", marginBottom: "0.75rem" }}>Ready to work together?</h3>
            ...
            <a href="mailto:joan.munyi3@gmail.com">
              <button style={{ background: "var(--accent)", color: "#fff", padding: "0.8rem 2rem", borderRadius: "8px", fontWeight: 500, fontSize: "0.95rem", border: "none", cursor: "pointer", fontFamily: "'Outfit',sans-serif" }}>
                Send me an email →
              </button>
            </a>
          </div>
          ...
          {contacts.map(({ label, value, href }) => (
            <div key={label} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "14px", padding: "1.5rem" }}>
              ...
              <div style={{ fontSize: "1rem", color: "var(--text)", fontWeight: 500 }}>
                {href
                  ? <a href={href} target="_blank" rel="noreferrer" style={{ color: "var(--accent2)", textDecoration: "none" }}>{value}</a>
                  : value}
              </div>
            </div>
          ))}
